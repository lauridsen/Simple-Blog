...
exports.getPosts = function (req, res, next) {
    if (!req.user) {
        ...
        Blog.find({}).sort({ blogCreated: 'desc' }).exec(function (error, blogposts) {
            res.render("blog", { title: "Blog", messages: req.flash("error"), posts: blogposts })
        });
    }
};
...
exports.getPost = function (req, res, next) {
    ...
    Blog.findOne({ _id: param }, function (error, singlePost) {
        ...
        res.render("post", { title: singlePost.blogTitle, messages: req.flash("error"), selectedPost: singlePost });
    });
}